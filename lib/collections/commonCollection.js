...
class CommonCollection {
    ...
    //----------------------------------------------------------------
    // TODO: Replace all adds with this
    _add(items) {
    ...
    //----------------------------------------------------------------
    // TODO: Replace all removes with this
    _remove(items) {
        this._onRemoving(items);
        this._items.remove.apply(this._items, items.map(it => it.item));
    ...
    remove(item) {
        ...
        this._items.remove.apply(this._items, toRemove.item);
        this._onRemoved(toRemove);
        return true;
    }
    
    removeAll(filter) {
        if(this._items.length <= 0 || _.isNull(filter) || _.isUndefined(filter)) {
            return false;
        }
