...
var sortByIndexReverse = function sortByIndexReverse(a, b) {
...
class ObservableCollection extends Array {
    ...
    copyWithin(target, start/*, end*/) {
        ...
        if(adds.length > 0) {
            this.emit('adding', adds);
        }
        
        if(removes.length > 0) {
            this.emit('removing', removes);
        }
        ...
        if(adds.length > 0) {
            this.emit('added', adds);
        }
        
        if(removes.length > 0) {
            this.emit('removed', removes);
        }
    ...
    slice(begin, end) {
        ...
        for(var i = start; i < end; ++i) {
            change.push({ item: this[i], index: i });
        }

        this.emit('removing', change);
        var result = slice.call(this, begin, end);
        this.emit('removed', change);
