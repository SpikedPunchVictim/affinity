...
ObservableCollection.prototype[Symbol.iterator] = () => this._items;
...
class Listener {
    ...
    subscribe(event, handler) {
        var hasSubscribed = event in this._subscriptions;
    ...
    unsubscribe() {
        for(var i = 0; i < this._source.length; ++i) {
            ...
            for(var event in Object.keys(this._subscriptions)) {
                let handlers = this._subscriptions[event];
                
                for(var h in handlers) {
                    current.off(event, event);
                }
            }
        }
    }
    ...
    _onAdded(items) {
        for(var i = 0; i < items.length; ++i) {
            ...
            for(var key in Object.keys(this._subscriptions)) {
                let value = this._subscriptions[key];
                
                for(var h in value) {
                    current.item.on(key, value);
                }
            }
        }
    }
