...
class CollectionValue {
   ...
   update(value) {
      if(value instanceof CollectionValue) {
         ...
         for(let i = 0; i < Math.max(this.length, value.length); ++i) {
            ...
            } else if(!this.at(i).equals(other.at(i))) {
               changes.push(this._createChangeReplace(this.at(i), other.at(i), i));
            }
         }
   ...
   clear() {
      var changes = []
      for(var i = Math.max(0, this.length - 1); i >= this.length - 1; --i) {
         changes.push(this._createChangeRemove(this._items[i], i));
      }
   ...
   _remove(items) {
      // We record the removals in reverse index order. This allows
      // clients to safely recreate the collection without additional
      // coputation.
      items.sort(sortByIndexReverse);
