...
class CollectionValue {
   ...
   update(value) {
      if(value instanceof CollectionValue) {
         ...
         for(let i = 0; i < Math.max(this.length, value.length); ++i) {
            if(i >= this.length) {
               changes.push(this._createChangeAdd(value.at(i), i));
            } else if(i >= value.length) {
               changes.push(this._createChangeRemove(this.at(i), i));
            } else if(!this.at(i).equals(value.at(i))) {
               changes.push(this._createChangeReplace(this.at(i), value.at(i), i));
            }
         }
   ...
   add() {
      ...
      for(var i = 0; i < arguments.length; ++i) {
         changes.push(this._createChangeAdd( arguments[i], this.length + i));
      }
   ...
   move(from, to) {
      ...
      return this._move(this._createChangeMove(this._items[from], from, to));
   }
   ...
   clear() {
      ...
      if(this.length != 0) {
         for(var i = Math.max(0, this.length - 1); i >= 0; --i) {
            changes.push(this._createChangeRemove(this._items.at(i), i));
         }
      }
   ...
   remove(item) {
      ...
      for(var i = 0; i < arguments.length; ++i) {
         ...
         if(itemIndex < 0) {
          ...
          changes.push(this._createChangeRemove(current, itemIndex));
      }
   ...
   removeAt(index) {
      ...
      var change = [];
      change.push(this._createChangeRemove(item, index));
   ...
   removeAll(filter) {
      ...
      for(var i = 0; i < this._items.length; ++i) {
         ...
         if(filter(currentItem, i, this)) {
            toRemove.push(this._createChangeRemove(currentItem, i));
         }
      }
   ...
   *--------------------------------------------------------------------*/
   applyChangeSet(changes) {
      ...
      for(let change in changes) {
         switch(change.event) {
            case events.add: {
               promises.push(this._add({ index: change.index, item: change.item.clone() }));
               break;
            }
            case events.move: {
               promises.push(this._move(change))
               break;
            }
            case events.remove: {
               let item = this._items.at(change.index);
               promsies.push(this._remove({ item: item, index: change.index }));
               break;
            }
            case events.replace: {
               promises.push(
            ...
            default:
               throw new Error('Unsupported CollectionValue change event');
         }
      }

      // Convert to functions for sequencing
      let acts = promises.map(p => () => p);
   ...
   _move(items) {
      ...
      return Rfc.new(items)
         ...
         .fulfill(req => {
            ...
            for(var i = 0; i < items.length; ++i) {
               let current = items[i];
               self._items.move(current.from, current.to);
            }
   ...
   *--------------------------------------------------------------------*/
   _replace(items) {
      ...
      return Rfc.new(items)
         ...
         .fulfill(req => {
            ...
            for(var i = 0; i < items.length; ++i) {
               let current = items[i];
               self._items.removeAt(current.index);
               self._items.insert(current.index, current.new);
            }
   ...
   /*---------------------------------------------------------------------
   * Creates a changelist entry for add
   * 
   * @param item {Value} The item to add
   * @param index {uint} The index where it will be added
   * @return A changelist entry containing:
   *           : event {string} The add event
   *           : item {Value} The added Value
   *           : index {uint} The index to add the value
   *--------------------------------------------------------------------*/
   _createChangeAdd(item, index) {
      return { event: events.add, item: item, index: index };
   }

   /*---------------------------------------------------------------------
   * Creates a changelist entry for remove
   * 
   * @param item {Value} The item to remove
   * @param index {uint} The index where it will be removed
   * @return A changelist entry containing:
   *           : event {string} The remove event
   *           : item {Value} The removed Value
   *           : index {uint} The index to remove the value
   *--------------------------------------------------------------------*/
   _createChangeRemove(item, index) {
      return { event: events.remove, item: item, index: index };
   }

   /*---------------------------------------------------------------------
   * Creates a changelist entry for move
   * 
   * @param item {Value} The item to move
   * @param from {uint} The source index of the value
   * @param to {uint} The destination index of the value
   * @return A changelist entry containing:
   *           : event {string} The move event
   *           : item {Value} The removed Value
   *           : from {uint} The source index of the value
   *           : to {uint} The destination index of the value
   *--------------------------------------------------------------------*/
   _createChangeMove(item, from, to) {
      return { event: events.move, item: item, from: from, to: to };
   }

   /*---------------------------------------------------------------------
   * Creates a changelist entry for replace
   * 
   * @param item {Value} The item to add
   * @param index {uint} The index where it will be added
   * @return A changelist entry containing:
   ...
   *           : index {uint} The index of the value
   *--------------------------------------------------------------------*/
   _createChangeReplace(oldValue, newValue, index) {
